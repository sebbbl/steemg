...
ui <- fluidPage(
  ...
  tabsetPanel(
    ###### statistics ######
    tabPanel("Overview|总览",
             wellPanel(
               h4("Posts in the latest 7 days"),
               plotOutput('stat_plotn7'),
               hr(),
               h4("Posts in total"),
               plotOutput('stat_plotn')
             )
             ),
...
tabPanel("自我介绍|CN-hello",
         sidebarLayout(
           sidebarPanel(
             ...
             wellPanel(
               h4("累计文章数量"),
               plotOutput('cnhello_plot3', height = 200),
               hr(),
               h4("累计获赞数量"),
               plotOutput('cnhello_plot1', height = 200),
               hr(),
               h4("累计文章收益"),
               plotOutput('cnhello_plot2', height = 200)
             )
           ...
           mainPanel(
             wellPanel(
               ...
               dataTableOutput('cnhello_dt1')
             ),
             wellPanel(
               h3("人物榜："),
               hr(),
               checkboxGroupInput(
                 inputId = "cnhelloa_colsel",
                 label   = "选择显示哪些列:",
                 choices = c('author', 'posts', 'payout','votes'),
                 selected = c('author', 'posts', 'payout','votes'),
                 inline = TRUE
...
tabPanel("高考作文|CN-gaokao",
         sidebarLayout(
           sidebarPanel(
             ...
             wellPanel(
               h4("累计文章数量"),
               plotOutput('cngaokao_plot3', height = 200),
               hr(),
               h4("累计获赞数量"),
               plotOutput('cngaokao_plot1', height = 200),
               hr(),
               h4("累计文章收益"),
               plotOutput('cngaokao_plot2', height = 200)
             )
           ...
           mainPanel(
             wellPanel(
               ...
               dataTableOutput('cngaokao_dt1')
             ),
             wellPanel(
               h3("人物榜："),
               hr(),
               checkboxGroupInput(
                 inputId = "cngaokaoa_colsel",
                 label   = "选择显示哪些列:",
                 choices = c('author', 'posts', 'payout','votes'),
                 selected = c('author', 'posts', 'payout','votes'),
                 inline = TRUE
